...
class VariantDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # get sequence
        seq = row["seq"]
        assert row["seq"][row["rel_pos"]] == row["Ref"]
        max_pos = len(seq) - self.kmer_size + 1
        # tokenize sequence
        tok = torch.tensor(self.tok_func(seq)["input_ids"])
        #print(tok)
        # mask position
        if pos < self.kmer_size - 1:
            tok[self.left_special_tokens:self.left_special_tokens+pos+1] = self.mask_token
        elif pos > max_pos:
            tok[self.left_special_tokens+(pos-self.kmer_size+1):len(tok)-self.right_special_tokens] = self.mask_token
        else:
            tok[self.left_special_tokens+(pos-self.kmer_size+1):self.left_special_tokens+pos+1] = self.mask_token
        return tok, torch.tensor(pos)
...
dl = torch.utils.data.dataloader.DataLoader(ds, num_workers=8, pin_memory=True, batch_size=48)

nucpreds = []

for batch in tqdm.tqdm(dl):
    tokens, positions = batch
    prbs = reconstructor(tokens.to(device)).cpu()
    nucpreds.append(prbs[torch.arange(prbs.shape[0]),positions,:])

nucpreds = torch.concat(nucpreds,axis=0)

adata = ad.AnnData(nucpreds.numpy(), 
                   obs=table,
                   var=pd.DataFrame({'Nucleotide':['A','C','G','T']}),
                  )
